<form action="{$VAL_SELF}" method="post" enctype="multipart/form-data">
	<div id="PaymentSense_Redirect" class="tab_content">
		<h3>{$TITLE}</h3>
		<h3>{$LANG.paymentsense.module_title}</h3>
		<p><b>Module Version:</b> {$LANG.paymentsense.module_version}<br /><b>Release Date:</b> {$LANG.paymentsense.module_date}</p>
		...
		<fieldset>
			...
			<p><em>{$LANG.paymentsense.merchantsettings_description}</em></p>
			<div>
				<label for="merchantID">{$LANG.paymentsense.merchantID}</label>
				<span>
				<input name="module[merchantID]" id="merchantID" class="textbox" type="text" value="{$MODULE.merchantID}" />
			</span></div>
			<div>
				<label for="merchantPass">{$LANG.paymentsense.merchantPass}</label>
				<span>
				<input name="module[merchantPass]" id="merchantPass" class="textbox" type="text" value="{$MODULE.merchantPass}" />
			</span></div>
			<div>
				<label for="merchantPSK">{$LANG.paymentsense.merchantPSK}</label>
				<span>
			...
			<div>
				...
				<select name="module[charge_type]">
					<option value="SALE" {$SELECT_charge_type_Auth}>{$LANG.paymentsense.mode_auth}</option>
					<option value="PREAUTH" {$SELECT_charge_type_PreAuth}>{$LANG.paymentsense.mode_preauth}</option>
				</select>
				</span> </div>
			<p><b>{$LANG.paymentsense.mandatoryfields_title}</b></p>
				<p><em>{$LANG.paymentsense.mandatoryfields_description}</em></p>
				<div>
					<label for="CV2Mandatory">{$LANG.paymentsense.mand_CV2}</label>
					<span>
					<input type="hidden" name="module[CV2Mandatory]" id="CV2Mandatory" class="toggle" value="{$MODULE.CV2Mandatory}" />
				</span></div>
				<div>
					<label for="Address1Mandatory">{$LANG.paymentsense.mand_Address1}</label>
					<span>
					<input type="hidden" name="module[Address1Mandatory]" id="Address1Mandatory" class="toggle" value="{$MODULE.Address1Mandatory}" />
				</span></div>
				<div>
					<label for="CityMandatory">{$LANG.paymentsense.mand_City}</label>
					<span>
					<input type="hidden" name="module[CityMandatory]" id="CityMandatory" class="toggle" value="{$MODULE.CityMandatory}" />
				</span></div>
				<div>
					<label for="PostCodeMandatory">{$LANG.paymentsense.mand_PostCode}</label>
					<span>
					<input type="hidden" name="module[PostCodeMandatory]" id="PostCodeMandatory" class="toggle" value="{$MODULE.PostCodeMandatory}" />
				</span></div>
				<div>
					<label for="StateMandatory">{$LANG.paymentsense.mand_State}</label>
					<span>
					<input type="hidden" name="module[StateMandatory]" id="StateMandatory" class="toggle" value="{$MODULE.StateMandatory}" />
				</span></div>
				<div>
					<label for="CountryMandatory">{$LANG.paymentsense.mand_Country}</label>
					<span>
